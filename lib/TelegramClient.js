...
const AuthKey = require('telegram-mt-node').auth.AuthKey;
...
class TelegramClient {
   ...
   * @param {Object} [authKey] -- user auth key
   * @param {String} [authKey.id] -- hex string (no leading 0x)
   * @param {String} [authKey.value] -- hex string (no leading 0x)
   */
  constructor(appId, hash, version, dataCenter, authKey) {
    this.appId = appId;
    this.hash = hash;
    ...
    if (authKey) {
     this.authKey = new AuthKey(
       Buffer.from(authKey.id, 'hex'),
       Buffer.from(authKey.value, 'hex')
     );
    }
  }
